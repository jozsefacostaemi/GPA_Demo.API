...
namespace Web.Core.Business.API.Infraestructure.Persistence.Repositories.Core
{
    public class HealthCareStaffRepository : IHealthCareStaffRepository
    {
        ...
        public async Task<RequestResult> SearchFirstHealCareStaffAvailable(string ProcessCode)
        {
            var getFirstHealCareStaffAvailable = await _context.HealthCareStaffs
                .Where(x => x.PersonState != null && x.Process.Code.Equals(ProcessCode) && x.PersonState.Code.Equals(PersonStateEnum.DISP.ToString()) && x.Loggued == true && x.AvailableAt != null).OrderBy(x => x.AvailableAt).FirstOrDefaultAsync();
