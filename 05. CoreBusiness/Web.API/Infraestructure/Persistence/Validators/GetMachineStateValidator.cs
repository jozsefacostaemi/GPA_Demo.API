...
namespace Web.Core.Business.API.Infraestructure.Persistence.Validators
{
    public class GetMachineStateValidator
    {
        ...
        private async Task<bool> CanNotCreateAttention(Guid Patient) => await _context.Attentions.Where(x => x.PatientId.Equals(Patient) && x.Open == true && x.Active == true).AnyAsync();
        #endregion
    }
}
